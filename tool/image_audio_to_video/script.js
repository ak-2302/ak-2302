...
async function load_ffmpeg(){
  ...
  ffmpeg=window.FFmpeg.createFFmpeg({log:false,corePath:'https://cdn.jsdelivr.net/npm/@ffmpeg/core@0.9.8/dist/ffmpeg-core.js'});
...
export_button.addEventListener('click',async()=>{if(!image_file||!audio_file)return;export_button.disabled=true;document.getElementById('export_label').textContent='書き出し中…';export_status.textContent='音声の長さに合わせて動画を生成しています';let audio_context;let source;try{audio_context=new AudioContext();source=audio_context.createMediaElementSource(audio_element);const destination=audio_context.createMediaStreamDestination();source.connect(destination);const canvas_stream=preview_canvas.captureStream(30);const stream=new MediaStream([...canvas_stream.getVideoTracks(),...destination.stream.getAudioTracks()]);const direct_mp4=MediaRecorder.isTypeSupported('video/mp4');const mime_type=direct_mp4?'video/mp4':(MediaRecorder.isTypeSupported('video/webm;codecs=vp9,opus')?'video/webm;codecs=vp9,opus':'video/webm;codecs=vp8,opus');const recorder=new MediaRecorder(stream,{mimeType:mime_type});const chunks=[];recorder.ondataavailable=e=>{if(e.data.size)chunks.push(e.data)};const finished=new Promise(resolve=>recorder.onstop=resolve);audio_element.currentTime=0;audio_element.play();recorder.start(250);const draw=()=>{draw_frame();if(!audio_element.paused){animation_id=requestAnimationFrame(draw)}};draw();audio_element.onended=()=>{recorder.stop();cancelAnimationFrame(animation_id);audio_element.pause()};await finished;let output_blob;if(direct_mp4){output_blob=new Blob(chunks,{type:'video/mp4'})}else{export_status.textContent='MP4へ変換しています。初回は少し時間がかかります';if(!window.crossOriginIsolated)throw new Error('このブラウザは静的ページでのMP4録画に対応していません。Chromeで専用サーバーを使うか、Safariで開いてください');const webm_blob=new Blob(chunks,{type:'video/webm'});const engine=await load_ffmpeg();const input_name=`input_${Date.now()}.webm`;const output_name=`output_${Date.now()}.mp4`;try{engine.FS('writeFile',input_name,await window.FFmpeg.fetchFile(webm_blob));await engine.run('-i',input_name,'-c:v','libx264','-preset','veryfast','-crf','26','-pix_fmt','yuv420p','-movflags','+faststart','-c:a','aac','-b:a','128k',output_name);const data=engine.FS('readFile',output_name);output_blob=new Blob([data.buffer],{type:'video/mp4'})}finally{try{engine.FS('unlink',input_name);engine.FS('unlink',output_name)}catch{}}}const url=URL.createObjectURL(output_blob);const link=document.createElement('a');link.href=url;link.download=`${(image_file.name||'video').split('.')[0]}_video.mp4`;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);source.disconnect();await audio_context.close();document.getElementById('export_label').textContent='動画を書き出す';export_status.textContent='MP4の書き出しが完了しました'}catch(error){console.error(error);export_status.textContent=`書き出しに失敗しました: ${error.message||'変換エラー'}`}finally{if(source)try{source.disconnect()}catch{}if(audio_context)try{await audio_context.close()}catch{}export_button.disabled=false}});

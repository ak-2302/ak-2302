...
function uniqueName(candidate, usedNames) {
  ...
  const base =
    extensionIndex > -1 ? candidate.slice(0, extensionIndex) : candidate;
...
function renderList() {
  ...
  state.files.forEach((entry, index) => {
    ...
    row.append(
      ...
      createCell(
        entry.duration === null ? "未取得" : formatDuration(entry.duration),
      ),
      createCell(
        entry.hashing
          ? "確認中"
...
function render() {
  ...
  const outputCount = elements.dedupe.checked
    ? state.files.length - duplicateCount
    : state.files.length;
  elements.count.textContent = String(state.files.length);
  elements.size.textContent = formatBytes(
    state.files.reduce((sum, entry) => sum + entry.file.size, 0),
  );
  ...
  elements.convertButton.disabled =
    ...
    state.files.some((entry) => entry.hashing);
  ...
  elements.downloadAllButton.textContent = state.archive
    ? "ZIPをダウンロード"
    : "すべてダウンロード";
  updateRenamePreview();
  renderList();
...
async function loadFfmpeg() {
  ...
  if (!window.FFmpeg)
    throw new Error("FFmpegライブラリを読み込めませんでした。");
...
async function buildArchive() {
  ...
  const baseName =
    sanitizeFileName(elements.zipName.value.replace(/\.zip$/i, "")) ||
    "converted-videos";
  state.archive = {
    blob,
    name: `${baseName}.zip`,
...
async function convertVideos() {
  ...
  try {
    ...
    for (let index = 0; index < entries.length; index += 1) {
      ...
      setProgress(
        percentBefore,
        `${index + 1}/${entries.length} ${entry.file.name}`,
      );
      ...
      try {
        ...
        for (const fsName of [inputName, outputFsName]) {
          try {
            engine.FS("unlink", fsName);
          } catch {
            /* File may not exist after a failed conversion. */
          }
        }
      }
      setProgress(
        Math.round(((index + 1) / entries.length) * 90),
        `${index + 1}/${entries.length} 完了`,
      );
    }

    if (!state.results.length)
      throw new Error("すべての動画の変換に失敗しました。");
...
elements.dropZone.addEventListener(
  "drop",
  (event) => void addFiles(event.dataTransfer.files),
);
...
elements.dedupe.addEventListener("change", () => {
  clearResults();
  render();
});
elements.zip.addEventListener("change", () => {
  clearResults();
  render();
});

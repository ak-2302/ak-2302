...
function get_settings(){const crop_ratio=$('crop_ratio').value;return {mime:$('save_format_select').value,quality:.85,width:Number($('width_input').value)||null,height:Number($('height_input').value)||null,lock:$('aspect_lock').checked,brightness:Number($('brightness_input').value),contrast:Number($('contrast_input').value),saturation:Number($('saturation_input').value),crop:crop_ratio!=='none',crop_ratio};}
function update_preview(){const image=$('canvas_image');if(!active_image||image.hidden)return;const settings=get_settings();image.style.filter=`brightness(${100+settings.brightness}%) contrast(${100+settings.contrast}%) saturate(${100+settings.saturation}%) drop-shadow(0 14px 20px #0008)`;image.style.transform=`translate(${active_image.pan_x}px,${active_image.pan_y}px) rotate(${active_image.rotation}deg) scale(${active_image.zoom*(active_image.flip_h?-1:1)},${active_image.zoom*(active_image.flip_v?-1:1)})`;image.classList.toggle('preview_crop',settings.crop);image.style.aspectRatio=settings.crop?settings.crop_ratio.replace(':',' / '):'';image.style.objectFit=settings.crop?'cover':'contain';if(settings.width||settings.height){const natural_width=image.naturalWidth,natural_height=image.naturalHeight;let width=settings.width,height=settings.height;if(width&&!height&&settings.lock)height=Math.round(width*natural_height/natural_width);if(height&&!width&&settings.lock)width=Math.round(height*natural_width/natural_height);image.style.width=width?`${width}px`:'';image.style.height=height?`${height}px`:'';}else{image.style.width='';image.style.height='';}}
function canvas_for(image,item,settings){
  let source_w=image.naturalWidth, source_h=image.naturalHeight;
  if(settings.crop){const [ratio_w,ratio_h]=settings.crop_ratio.split(':').map(Number);const source_ratio=source_w/source_h,target_ratio=ratio_w/ratio_h;let crop_w=source_w,crop_h=source_h;if(source_ratio>target_ratio)crop_w=Math.round(source_h*target_ratio);else crop_h=Math.round(source_w/target_ratio);const sx=Math.round((source_w-crop_w)/2),sy=Math.round((source_h-crop_h)/2);const crop=document.createElement('canvas');crop.width=crop_w;crop.height=crop_h;crop.getContext('2d').drawImage(image,sx,sy,crop_w,crop_h,0,0,crop_w,crop_h);image=crop;source_w=crop_w;source_h=crop_h;}
...
$('clear_button').addEventListener('click',()=>{files.splice(0).forEach(item=>URL.revokeObjectURL(item.url));results.splice(0).forEach(item=>URL.revokeObjectURL(item.url));render_files();result_panel.hidden=true;set_status('');});
 const save_modal=$('save_modal'),save_form=$('save_form'),save_format_select=$('save_format_select'),filename_input=$('filename_input');
 function open_save_modal(){if(!files.length)return;const base=files[0].file.name.replace(/\.[^.]+$/,'');filename_input.value=base;save_modal.hidden=false;filename_input.focus();}
 function close_save_modal(){save_modal.hidden=true;}
 convert_button.addEventListener('click',open_save_modal);$('close_save_modal').addEventListener('click',close_save_modal);$('cancel_save').addEventListener('click',close_save_modal);save_modal.addEventListener('click',event=>{if(event.target===save_modal)close_save_modal();});
 save_form.addEventListener('submit',async event=>{event.preventDefault();const original_name=files[0].file.name;files[0].file=new File([files[0].file],`${filename_input.value.trim()}.${extension_for(save_format_select.value)}`,{type:files[0].file.type});await convert();files[0].file=new File([files[0].file],original_name,{type:files[0].file.type});close_save_modal();});$('download_all_button').addEventListener('click',async()=>{if(!results.length)return;const zip=new JSZip();results.forEach(result=>zip.file(result.name,result.blob));const blob=await zip.generateAsync({type:'blob'});const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download='converted-images.zip';link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});
 [['brightness','0'],['contrast','0'],['saturation','0']].forEach(([name])=>$(name+'_input').addEventListener('input',e=>{$(name+'_value').textContent=e.target.value;update_preview();}));
$('width_input').addEventListener('input',()=>{if(active_image?.image&&$('aspect_lock').checked&&$('width_input').value)$('height_input').value=Math.round(Number($('width_input').value)*active_image.image.naturalHeight/active_image.image.naturalWidth);update_preview();});$('height_input').addEventListener('input',()=>{if(active_image?.image&&$('aspect_lock').checked&&$('height_input').value)$('width_input').value=Math.round(Number($('height_input').value)*active_image.image.naturalWidth/active_image.image.naturalHeight);update_preview();});$('aspect_lock').addEventListener('change',update_preview);$('crop_ratio').addEventListener('change',update_preview);

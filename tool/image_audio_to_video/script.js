...
export_button.addEventListener('click',async()=>{if(!image_file||!audio_file)return;export_button.disabled=true;document.getElementById('export_label').textContent='書き出し中…';export_status.textContent='音声の長さに合わせて動画を生成しています';const audio_context=new AudioContext();const source=audio_context.createMediaElementSource(audio_element);const destination=audio_context.createMediaStreamDestination();source.connect(destination);const canvas_stream=preview_canvas.captureStream(30);const stream=new MediaStream([...canvas_stream.getVideoTracks(),...destination.stream.getAudioTracks()]);const recorder=new MediaRecorder(stream,{mimeType:'video/webm;codecs=vp9,opus'});const chunks=[];recorder.ondataavailable=e=>{if(e.data.size)chunks.push(e.data)};const finished=new Promise(resolve=>recorder.onstop=resolve);audio_element.currentTime=0;audio_element.play();recorder.start(250);const draw=()=>{draw_frame();if(!audio_element.paused){animation_id=requestAnimationFrame(draw)}};draw();audio_element.onended=()=>{recorder.stop();cancelAnimationFrame(animation_id);audio_element.pause()};await finished;const blob=new Blob(chunks,{type:'video/webm'});const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download=`${(image_file.name||'video').split('.')[0]}_video.webm`;link.click();URL.revokeObjectURL(url);source.disconnect();await audio_context.close();export_button.disabled=false;document.getElementById('export_label').textContent='動画を書き出す';export_status.textContent='書き出しが完了しました。ダウンロードを確認してください'});

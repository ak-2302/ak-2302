...
でリポジトリのコミット一覧を表示し選択可能。直近100件まで取得する。

現在の実装ではPages URL入力後にGitHub APIから直近100件を自動取得する。
コミットを選択しない場合は、入力されたGitHub Pages URLの最新公開ページをそのまま表示する。
表示中のページは新しいタブで開ける。
...
- 公開リポジトリのみ対応
- コミット履歴はGitHub APIの未認証レート制限の対象
- 履歴一覧はリポジトリのデフォルトブランチにある直近30件

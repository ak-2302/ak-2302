(function () {
  ...
  if (!window.THREE || !window.Physijs) {
    ...
    if (loader)
      loader.querySelector("p").textContent = "3D CONTENT COULD NOT LOAD";
    return;
  }

  Physijs.scripts.worker = new URL(
    "./ref/script/vendor/physijs_worker.js",
    window.location.href,
  ).href;
  Physijs.scripts.ammo = new URL(
    "./ref/script/vendor/ammo.js",
    window.location.href,
  ).href;
  ...
  renderer.domElement.setAttribute(
    "aria-label",
    "PROFILE、TOOL、NOTE、IDEA、LINK、CONTACTの6つの球",
  );
  ...
  renderer.domElement.addEventListener("pointermove", (event) => {
    ...
    if (activePointers.size === 2) {
      ...
      if (pinchDistance) {
        orbit.radius = THREE.Math.clamp(
          orbit.radius - (distance - pinchDistance) * 0.025,
          12,
          24,
        );
        updateCamera();
    ...
    if (drag.moved && !drag.selected) {
      boxRotation.x = THREE.Math.clamp(
  ...
  function finishPointer(event) {
    ...
    const shouldOpen =
      activePointers.size === 1 && !drag.opened && !drag.moved && wasSelected;
  ...
  function render() {
    ...
    sphereLabels.forEach((item) => {
      cameraDirection
        .subVectors(localCameraPosition, item.sphere.position)
        .normalize();
      item.sprite.position
        .copy(item.sphere.position)
        .add(cameraDirection.multiplyScalar(item.radius * 1.015));
    });

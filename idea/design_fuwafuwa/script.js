...
function reset_page() {
  ...
  document
    .querySelectorAll(".bubble.bursting")
    .forEach((element) => element.remove());
  bubble_count.textContent = bubbles.length;
}

function tick(time) {
  const dt = Math.min((time - last_time) / 1000, 0.04);
  last_time = time;
  quiet_time += dt;
  const settling = Math.max(0.08, Math.exp(-quiet_time / 10));
  for (const bubble of bubbles) {
    const dx = 50 - bubble.x;
    const dy = 50 - bubble.y;
    bubble.vx += dx * (0.16 * settling) * dt;
    bubble.vy += dy * (0.16 * settling) * dt;
    for (const other of bubbles) {
      if (bubble === other) continue;
      const ox = bubble.x - other.x;
      const oy = bubble.y - other.y;
      ...
      if (distance < minimum) {
        ...
        bubble.x += (ox / distance) * overlap * 0.12;
        bubble.y += (oy / distance) * overlap * 0.12;
    ...
    if (distance_from_center > 44) {
      bubble.vx += dx * 0.03 * dt;
      bubble.vy += dy * 0.03 * dt;
    }
    const damping = 0.962 + settling * 0.023;
    bubble.vx *= damping;
    bubble.vy *= damping;
    bubble.x += bubble.vx * dt * 10;
...
function add_bubble() {
  quiet_time = 0;
  create_bubble(5 + Math.random() * 5, Math.random() * Math.PI * 2, 39);
  bubble_count.textContent = bubbles.length;
}
for (let i = 0; i < 18; i++)
  create_bubble(
